...
class SubGrapher:
    ...
    def get_depth(self, nodes):
        ...
        for node in nodes:
            distance+=1
            # distance += self.graph["nodes"][node - 1]["distance"]
...
#     def get_similar_nodes(self, sentence: str):
#         sentence_encoding = self.encode(
#             [
#                 sentence,
#             ]
...
#     def get_subgraph(self, person=None):
#         if person == None:
#             person = self.person
#         sentence = person.__str__()
#         sim_nodes = self.get_similar_nodes(sentence)
#         sim_edges = self.get_similar_edges(sim_nodes)
#         rec_nodes, breadth = self.get_uncompleted_nodes_breadth(sim_nodes, "1")
#         proficiency = self.get_topic_proficiency(sim_nodes)
#         depth = self.get_depth(sim_nodes)
...
#             prob_nodes.remove(idx)
#             insort(one_step_nodes, idx)
#         return one_step_nodes, breadth / total_breadth
